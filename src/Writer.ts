...
// resampled (see SF3EncodeResult). Already-compressed (SF3) samples pass
// through untouched. Up to `concurrency` samples are encoded at once: each
// worker pulls the next unencoded index off a shared counter and calls
// `encode`, so slow/uneven samples don't stall the whole batch the way a
// fixed chunk-based split would.
